<?php
	...
	*/
	...
	function get_table_groups($skip_authentication = false) {
		...
		$all_groups = ['None', 'Sales', 'Operations', 'Catalog'];
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'Orders' => "`Orders`.`OrderID` as 'OrderID', IF(    CHAR_LENGTH(`Customers1`.`CustomerName`), CONCAT_WS('',   `Customers1`.`CustomerName`), '') as 'Customer', IF(    CHAR_LENGTH(`Employees1`.`EmployeeName`), CONCAT_WS('',   `Employees1`.`EmployeeName`), '') as 'Employee', IF(    CHAR_LENGTH(`Products1`.`ProductName`), CONCAT_WS('',   `Products1`.`ProductName`), '') as 'Product', if(`Orders`.`OrderDate`,date_format(`Orders`.`OrderDate`,'%m/%d/%Y %H:%i'),'') as 'OrderDate'",
			'Products' => "`Products`.`ProductNo` as 'ProductNo', `Products`.`ProductName` as 'ProductName', `Products`.`Manufacturer` as 'Manufacturer'",
			'Customers' => "`Customers`.`CustomerID` as 'CustomerID', `Customers`.`CustomerName` as 'CustomerName', `Customers`.`Address` as 'Address', `Customers`.`State` as 'State', `Customers`.`Country` as 'Country'",
			'Employees' => "`Employees`.`EmployeeID` as 'EmployeeID', `Employees`.`EmployeeName` as 'EmployeeName', `Employees`.`SSN` as 'SSN'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'Orders' => "`Orders` LEFT JOIN `Customers` as Customers1 ON `Customers1`.`CustomerID`=`Orders`.`Customer` LEFT JOIN `Employees` as Employees1 ON `Employees1`.`EmployeeID`=`Orders`.`Employee` LEFT JOIN `Products` as Products1 ON `Products1`.`ProductNo`=`Orders`.`Product` ",
			'Products' => "`Products` ",
			'Customers' => "`Customers` ",
			'Employees' => "`Employees` ",
		];
