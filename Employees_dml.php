...
function Employees_insert(&$error_message = '') {
	...
	$data = [
		'EmployeeName' => Request::val('EmployeeName', ''),
		'SSN' => Request::val('SSN', ''),
	];
	...
	// enforce pk zerofill
	$recID = str_pad($recID, sqlValue("SELECT LENGTH(`EmployeeID`) FROM `Employees` LIMIT 1"), '0', STR_PAD_LEFT);
...
function Employees_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	$rires = sql("SELECT COUNT(1) FROM `Orders` WHERE `Employee`='" . makeSafe($EmployeeID[0]) . "'", $eo);
...
function Employees_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		$jsReadOnly = '';
		$jsReadOnly .= "\tjQuery('#EmployeeName').replaceWith('<div class=\"form-control-static\" id=\"EmployeeName\">' + (jQuery('#EmployeeName').val() || '') + '</div>');\n";
		$jsReadOnly .= "\tjQuery('#SSN').replaceWith('<div class=\"form-control-static\" id=\"SSN\">' + (jQuery('#SSN').val() || '') + '</div>');\n";
	...
	$templateCode = str_replace('<%%UPLOADFILE(EmployeeName)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(SSN)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(EmployeeName)%%>', safe_html($urow['EmployeeName']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(EmployeeName)%%>', html_attr($row['EmployeeName']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(EmployeeName)%%>', urlencode($urow['EmployeeName']), $templateCode);
		if( $dvprint) $templateCode = str_replace('<%%VALUE(SSN)%%>', safe_html($urow['SSN']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(SSN)%%>', html_attr($row['SSN']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(SSN)%%>', urlencode($urow['SSN']), $templateCode);
	} else {
		...
		$templateCode = str_replace('<%%VALUE(EmployeeName)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(EmployeeName)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(SSN)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(SSN)%%>', urlencode(''), $templateCode);
	}
