	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'Orders' => ['Orders', '', 'table.gif', 'Sales'],
			'Products' => ['Products', '', 'table.gif', 'Catalog'],
			'Customers' => ['Customers', '', 'table.gif', 'Sales'],
			'Employees' => ['Employees', '', 'table.gif', 'Operations'],
		];
	...
	function get_table_fields($tn = null) {
		...
		if($schema === null) {
			...
			$schema = [
				'Orders' => [
					'OrderID' => [
						'appgini' => "INT(10) ZEROFILL NOT NULL PRIMARY KEY AUTO_INCREMENT",
						'info' => [
							'caption' => 'OrderID',
							'description' => '',
						],
	...
	*/
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			'Orders' => [
				'Customer' => 'SELECT `Customers`.`CustomerID`, `Customers`.`CustomerName` FROM `Customers` ORDER BY 2',
				'Employee' => 'SELECT `Employees`.`EmployeeID`, `Employees`.`EmployeeName` FROM `Employees` ORDER BY 2',
				'Product' => 'SELECT `Products`.`ProductNo`, `Products`.`ProductName` FROM `Products` ORDER BY 2',
			],
			'Products' => [
			],
			'Customers' => [
